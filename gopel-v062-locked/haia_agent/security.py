...
class HashWitness:
    ...
    def __init__(self, witness_path: Path, witness_interval: int = 10,
        ...
        Args:
            ...
            witness_hmac_key: HMAC key for witness file authentication.
                If provided, _persist() writes an HMAC tag and __init__
                verifies it on load. SEC-06: prevents silent witness
                file replacement without detection.
        """
        self.witness_path = Path(witness_path)
        self.witness_interval = witness_interval
        self._witness_hmac_key = witness_hmac_key
        self._witnesses: list[dict] = []

        if self.witness_path.exists():
            with open(self.witness_path, "r") as f:
                data = json.load(f)
            # SEC-06: verify HMAC if key is configured
            if self._witness_hmac_key:
                stored_mac = data.get("hmac", "")
                witnesses_json = json.dumps(
                    data.get("witnesses", []), sort_keys=True
                )
                expected_mac = hmac.new(
                    self._witness_hmac_key.encode(),
                    witnesses_json.encode(),
                    hashlib.sha256,
                ).hexdigest()
                if not hmac.compare_digest(stored_mac, expected_mac):
                    raise ValueError(
                        "Witness file HMAC verification failed: "
    ...
    def _persist(self) -> None:
        ...
        SEC-06: If witness_hmac_key is configured, computes HMAC-SHA256
        over the serialized witnesses array and stores it in the file.
        Prevents undetected witness file replacement.
        """
        ...
        # SEC-06: compute and store HMAC over witnesses
        if self._witness_hmac_key:
            witnesses_json = json.dumps(self._witnesses, sort_keys=True)
            data["hmac"] = hmac.new(
                self._witness_hmac_key.encode(),
                witnesses_json.encode(),
                hashlib.sha256,
...
class AuditEncryption:
    ...
    def decrypt(self, ciphertext: str) -> str:
        ...
        SEC-02: Obfuscation fallback now verifies HMAC tag before returning.
        """
        if self._fernet:
            return self._fernet.decrypt(ciphertext.encode()).decode()
        else:
            # Fallback: decode base64 with HMAC verification
            if ciphertext.startswith("OBFUSCATED:"):
                parts = ciphertext.split(":", 2)
                stored_tag = parts[1]
                encoded_data = parts[2]
                plaintext = base64.b64decode(encoded_data).decode()
                # SEC-02: Verify HMAC tag to detect tampering
                expected_tag = hmac.new(
                    self._key.encode(), plaintext.encode(), hashlib.sha256
                ).hexdigest()[:16]
                if not hmac.compare_digest(stored_tag, expected_tag):
                    raise ValueError(
                        "HMAC verification failed: audit data may have been tampered with"
                    )
                return plaintext
            return ciphertext
...
class AuditFileLock:
    ...
    @contextmanager
    def read_lock(self, verify_after_read: Optional[Callable] = None):
        """
        Acquire a shared read lock on the audit file.
        Allows concurrent reads but blocks during writes.
        Uses a local file descriptor (thread-safe).

        SEC-09: Optional verify_after_read callback. When provided,
        the caller invokes it while the lock is still held to verify
        chain integrity of the data just read. This closes the
        TOCTOU window between lock acquisition and data consumption.

        Args:
            verify_after_read: Callable that returns (bool, str).
                Called inside the lock context. If it returns
                (False, reason), a ValueError is raised before
        ...
        try:
            ...
            yield
            # SEC-09: post-read verification while lock is still held
            if verify_after_read is not None:
                is_valid, reason = verify_after_read()
                if not is_valid:
                    raise ValueError(
                        f"Post-read chain integrity check failed: {reason}"
                    )
        finally:
            fcntl.flock(lock_fd.fileno(), fcntl.LOCK_UN)
            lock_fd.close()
