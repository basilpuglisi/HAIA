...
class EvidenceSubmission:
    ...
    def signable_payload(
        self, transaction_id: str = "", gate_name: str = ""
    ) -> dict:
        """Produce the dict that should be signed for verification.
        SEC-03: Includes transaction_id and gate_name to prevent
        cross-gate replay of approval signatures.
        SEC-10: Renamed from _signable_payload to public API.
        """
        return {
...
class EvidenceGate:
    ...
    def check(
        ...
        transaction_id: str = "",  # SEC-03: bind signatures to transaction
    ) -> EvidenceGateResult:
        """
...
class EscalationEngine:
    ...
    def evaluate(self, context: GovernanceContext) -> EscalationState:
        ...
        for rule in self._rules:
            # SEC-05: Fail-closed on rule exception (mirrors GV5 in PolicyEngine)
            try:
                rule_triggered = rule.condition(context)
            except Exception as e:
                # Rule crashed: escalate to LOCKDOWN (fail-closed)
                triggered_level = EscalationLevel.LOCKDOWN
                triggered_reason = (
                    f"Rule '{rule.name}' crashed during evaluation: {e}. "
                    f"Fail-closed escalation to LOCKDOWN."
                )
                triggered_role = OperatorRole.ADMINISTRATOR
                break  # Cannot trust further rule evaluation
            if rule_triggered:
...
class GovernanceRuntime:
    ...
    def __init__(
        ...
        # SEC-08: Use deque with maxlen for O(1) eviction instead of
        # list.pop(0) which is O(n). GV6: Bound log size.
        self._decision_log: collections.deque[GovernanceDecision] = (
            collections.deque(maxlen=max_decision_log)
        )
        self._max_decision_log = max_decision_log
    ...
    def escalation_engine(self) -> EscalationEngine:
        """SEC-07: Public accessor for the escalation engine.

        Allows external monitors (Overwatch) to evaluate escalation
        context without accessing private internals.
        """
    ...
    def _log_decision(self, decision: GovernanceDecision) -> None:
        ...
        """
        # SEC-08: deque(maxlen=N) auto-evicts oldest on append (O(1)).
        # Track eviction count before append for audit awareness.
