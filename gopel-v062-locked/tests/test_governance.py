...
def test_t1b_signed_second_approver_passes():
    ...
    # SEC-03: Sign with transaction context to prevent cross-gate replay
    tid = "test-transaction-001"
    gate = CIRCUIT_BREAKER_OVERRIDE_GATE.gate_name
    approval.signature = approver_identity.sign_record(
        approval.signable_payload(transaction_id=tid, gate_name=gate)
    )
...
def test_t1b_wrong_key_signature_rejected():
    ...
    tid = "test-transaction-forged"
    gate = CIRCUIT_BREAKER_OVERRIDE_GATE.gate_name
    approval.signature = forger.sign_record(
        approval.signable_payload(transaction_id=tid, gate_name=gate)
    )
